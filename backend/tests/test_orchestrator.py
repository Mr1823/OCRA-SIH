...
LLM API key (uses keyword-based fallback).
"""
...
# tests/conftest.py already blanks API keys before config.py loads; force the
# cached attribute too, so orchestrator/synthesis reliably take the
